    ...
    <div class="week">
        ...
        <div v-show="!week.folded" class="week-text">
            <div class="week-info week-text-child">  
                ...
                <a :key="doc.fileNameBack" v-for="doc in week.docs" class="week-doc" :href="'./' + week.cours + '/' + doc.fileNameBack" target="_blank">{{ doc.fileNameFront }}<br></a>
            </div>
            <div class="week-todo week-text-child">
                <button class="addTaskButton" @click="toggleTaskForm">
                    <i class="material-icons" >add_circle</i>
                </button>
                <form class="taskForm" v-show="addTask"  @submit.prevent="newTask(this.week)">
                    <input ref="taskInput" id="taskInput" class="form-control" type="text" v-model="taskName" name="taskName">
                </form>
...
export default {
    ...
    methods: {
        ...
        async checkMe(task) {
            ...
            let index = data.tasks.findIndex(item => item.taskName === task.taskName)
